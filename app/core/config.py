...
class Settings(BaseSettings):
    ...
    STAGE_3_MODEL: str = Field(default="gpt-4o", description="Model for Stage 3 role compatibility (high-quality screening)")
    
    # Stage 3 Role Compatibility Template - System Message
    ROLE_COMPATIBILITY_SYSTEM_MESSAGE: str = Field(
        default="You are an executive recruiter specializing in C-level technology leadership roles (CIO, CTO, CISO). Your task is a fast triage to decide whether a candidate profile is viable for Fortium Partners' partner network before running full deterministic scoring. Be strict, structured, and evidence-based.\n\nImportant:\n- Always return ALL required keys exactly as defined.\n- Arrays (key_qualifications, missing_qualifications) must never be empty. If evidence is missing, insert a descriptive gap (e.g., 'No clear evidence of global scope').\n- Never invent details beyond what the profile shows, but always produce a concise reason for each qualification and gap.\n- For overall_assessment, always begin with a compact heatmap summary of compatibility scores in the form: \"CIO X.XX | CTO X.XX | CISO X.XX.\" Then follow with 2–4 sentences of narrative assessment.\n- Use JSON-only output with no extra commentary.",
        description="System message for Stage 3 role compatibility checking"
    )
    
    # Stage 3 Role Compatibility Template - User Message  
    ROLE_COMPATIBILITY_USER_MESSAGE: str = Field(
        default="Analyze this LinkedIn profile JSON and determine which executive role (CTO, CIO, CISO) this person is best suited for, if any. Then produce the exact response format below.\n\n**Role Definitions:**\n- CTO: Technical leadership for product/engineering in Technology Creators (software/SaaS). Builds and scales product/engineering; reports to CEO; product P&L influence.\n- CIO: Enterprise IT leader in Technology Consumers. IT strategy, ERP/CRM, infra/ops, vendor/spend alignment, digital transformation; reports to CEO/COO/CFO.\n- CISO: Enterprise security/risk leader. Governance, risk, compliance, IR, IAM, cloud security; board visibility; reports to CEO/COO/CFO (not buried under IT).\n\n**Evaluation Criteria:**\n1) Experience Match 2) Leadership Level 3) Technical/Security Depth 4) Strategic Focus.\nTreat ambiguous titles (CDO, Head of IT, VP Security) as non-qualifying unless trajectory clearly reaches CIO/CTO/CISO accountability.\n\n**Scoring & Gate:**\n- Produce compatibility scores in [0.0, 1.0] for each of CTO/CIO/CISO.\n- Minimum executive threshold for a role = 0.40.\n- passes_gate = true if the recommended role >= 0.40 AND there is evidence of top accountability and enterprise scope.\n- proceed_with_scoring = true if passes_gate is true; otherwise false.\n\n**Candidate Data:**\n{{profile_data}}\n\n**Response Format (JSON only, exact keys; arrays must be non-empty; strings must be non-empty):**\n{\n  \"proceed_with_scoring\": true|false,\n  \"compatible_roles\": [\n    {\n      \"role\": \"CTO\",\n      \"compatible\": true|false,\n      \"confidence\": 0.0-1.0,\n      \"reasoning\": \"1-2 sentences\",\n      \"key_qualifications\": [\"2-4 concise bullets\"],\n      \"missing_qualifications\": [\"2-4 concise gaps/risks\"]\n    },\n    {\n      \"role\": \"CIO\",\n      \"compatible\": true|false,\n      \"confidence\": 0.0-1.0,\n      \"reasoning\": \"1-2 sentences\",\n      \"key_qualifications\": [\"2-4 concise bullets\"],\n      \"missing_qualifications\": [\"2-4 concise gaps/risks\"]\n    },\n    {\n      \"role\": \"CISO\",\n      \"compatible\": true|false,\n      \"confidence\": 0.0-1.0,\n      \"reasoning\": \"1-2 sentences\",\n      \"key_qualifications\": [\"2-4 concise bullets\"],\n      \"missing_qualifications\": [\"2-4 concise gaps/risks\"]\n    }\n  ],\n  \"recommended_primary_role\": \"CTO\" | \"CIO\" | \"CISO\" | \"NONE\",\n  \"overall_assessment\": \"Must start with: 'CIO X.XX | CTO X.XX | CISO X.XX.' Then add 2–4 sentences summarizing the choice and key signals\"\n}",
        description="User message template for Stage 3 role compatibility checking"
    )
